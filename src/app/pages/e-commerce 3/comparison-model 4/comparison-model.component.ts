...
export class ComparisonModelComponent4 {
  ...
  fileName = "OB_GTN_AAS_&_CostLog_Comparison"
  ...
  async downloadExcel()
  {
    ...
    let title =["","OB Table","","GTN Table","","AA Summary","","","","","Cost Log","" ]
    ...
    let header=[
      ...
      "Unit Price", //d
      "", //e
      "FOB and Reviced FOB Price", //f
      "", //g
      "MMX", //h
      "BUR", //i
      "ROSS", //j
      "MMX or BUR or ROSS" //k
    ]
    ...
    for (let i = 0; i < OORMatchedOBRows.length; i =i + 1){
      ...
      if(JSON.stringify(OORMatchedOBRows[i].FOBPrice) !== JSON.stringify(OORMatchedOBRows[i].UnitPrice)){
        if(JSON.stringify(OORMatchedOBRows[i].UnitPrice) !== JSON.stringify(OORMatchedOBRows[i].FOBorRFOB)){
          if(JSON.stringify(OORMatchedOBRows[i].FOBorRFOB) !== JSON.stringify(OORMatchedOBRows[i].CLLogic)){
