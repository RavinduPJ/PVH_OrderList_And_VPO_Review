...
export class ComparisonModelComponent3 {
  ...
  fileName = "OB_AAS_&_GTN_Comparison"
  ...
  async downloadExcel()
  {
    ...
    for (const row of BffPVHSSArray) {
      ...
      if (OBMatchedRow2) {
        ...
        CFMSailBy = OBMatchedRow2.CFMSailBy,
    ...
    for (let i = 0; i < OORMatchedOBRows.length; i =i + 1){
      //prodware house
      if(JSON.stringify(OORMatchedOBRows[i].ReqDelDateComp) !== JSON.stringify(OORMatchedOBRows[i].ShipWindowEndDate)){
        if(JSON.stringify(OORMatchedOBRows[i].ShipWindowEndDate) !== JSON.stringify(OORMatchedOBRows[i].CFMSailBy)){
      ...
      if(JSON.stringify(OORMatchedOBRows[i].PlanDelDateComp) !== JSON.stringify(OORMatchedOBRows[i].ShipWindowEndDate)){
        if(JSON.stringify(OORMatchedOBRows[i].ShipWindowEndDate) !== JSON.stringify(OORMatchedOBRows[i].CFMSailBy)){
          worksheet.getCell("I"+(3+i)).fill = {
            type: 'pattern',
            pattern: 'solid',
      ...
      if(JSON.stringify(OORMatchedOBRows[i].COQty) !== JSON.stringify(OORMatchedOBRows[i].Units)){
